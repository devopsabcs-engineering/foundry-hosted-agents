...
foreach ($name in @("snet-aca-$stage", "snet-agent-$stage", 'snet-private-endpoints')) {
...
$account = @($accounts.value | Where-Object { $_.name -eq "aif-$EnvironmentName" })
if ($account.Count -gt 0) {
    ...
    if ($injections.Count -ne 1 -or
        $injections[0].subnetArmId -ne "$($network.id)/subnets/snet-agent-$stage" -or
