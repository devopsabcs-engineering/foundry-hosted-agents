...
class EvidenceConverter(ResponseAPIDefaultConverter):
    async def convert_request(self, context):
        if context.agent_run.conversation_id or context.agent_run.request.get("previous_response_id"):
            raise ValueError("This pilot requires full user/assistant history in input, store=false, "
                             "and no conversation or previous_response_id.")
        if context.agent_run.request.get("store") is True:
            raise ValueError("This pilot does not persist responses; send store=false.")
        return await super().convert_request(context)
